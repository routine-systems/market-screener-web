...
class HistoryTests(unittest.TestCase):
    def test_api_is_market_scoped_read_only(self):
        url = (
            ...
            + base64.b64encode(
                (ROOT / "functions/api/shortlist-history.js").read_bytes()
            ).decode()
        )
...
console.log(JSON.stringify({{calls,results}}));"""
        ...
        self.assertEqual(
            r["calls"], ["shortlist-history:v1:IN", "shortlist-history:v1:US"]
        )
...
"""
        ...
        r = json.loads(
            subprocess.check_output(
                ["node", "-e", prefix + script + fixture], text=True
            )
        )
        self.assertEqual(r["old"], ["OLD"])
        self.assertEqual(r["filtered"], 0)
        self.assertEqual(r["latest"], ["LIVE"])
    ...
    def test_history_controls_and_fallback_are_in_rendered_template(self):
        ...
        for expected in [
            ...
            "Reconstructed historical top 20",
        ]:
            self.assertIn(expected, s)
        self.assertIn("state.period='latest'", s)
        self.assertIn('${esc(p.date)}</option>', s)
        self.assertIn('${esc(liveDate())||\'—\'}</option>', s)
